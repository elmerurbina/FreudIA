...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def update_profile(self, request, pk=None):
        ...
        user.estado_psicologico = request.data.get(
            'estado_psicologico', user.estado_psicologico
        )
        user.save()
    ...
    def reset_password(self, request, pk=None):
        ...
        if user != request.user:
            return Response(
                {"detail": "No tienes permiso para cambiar la contraseña."},
                status=status.HTTP_403_FORBIDDEN,
            )
...
class PaymentViewSet(viewsets.ModelViewSet):
    ...
    def update_payment(self, request, pk=None):
        ...
        if payment.user != request.user:
            return Response(
                {"detail": "No tienes permiso para editar este pago."},
                status=status.HTTP_403_FORBIDDEN,
            )

        payment.card_number = request.data.get('card_number', payment.card_number)
        payment.expiration_date = request.data.get(
    ...
    def delete_payment(self, request, pk=None):
        ...
        return Response(
            {"detail": "Información de pago eliminada correctamente."},
    ...
    def get_user_payment(self, request):
        ...
        if not payment:
            return Response(
                {"detail": "No se encontraron detalles de pago para este usuario."},
    ...
    def create_payment(self, request):
        ...
        if hasattr(user, 'payment'):
            return Response(
                {"detail": "Ya existe una información de pago para este usuario."},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    if 'biometric_token' in request.data:
        ...
        try:
            user = User.objects.get(
                username=username
            )  # Or fetch based on biometric token
            refresh = RefreshToken.for_user(user)
            return Response(
                {
                    'refresh': str(refresh),
            ...
            return Response(
                {"detail": "User not found or invalid biometric data."},
                status=status.HTTP_401_UNAUTHORIZED,
            )

    return Response(
        {"detail": "Provide credentials or biometric data."},
        status=status.HTTP_400_BAD_REQUEST,
    )
